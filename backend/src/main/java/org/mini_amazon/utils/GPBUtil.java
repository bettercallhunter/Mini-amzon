...
public class GPBUtil {
  ...
  public static synchronized boolean send(
    ...
    try {
      CodedOutputStream codedOutputStream = CodedOutputStream.newInstance(
        outputStream
      );
  ...
  public static synchronized <T extends Message.Builder> T receiveFrom(
    T response,
  ...
  public static synchronized <T extends Message.Builder> T receive(
    T response,
    CodedInputStream codedInputStream
  ) throws IOException {
    byte[] bytes = codedInputStream.readByteArray();
    if (bytes.length == 0) {
